...
class SignUpForm(Form):
    username = StringField(
        ...
        [validators.DataRequired(utils.gettext("user name is required")),
         validators.Length(3, 32, utils.gettext("user name must be between 3 and 32 charactors long"))],
    ...
    password = PasswordField(
        ...
        [validators.Length(8, 32, utils.gettext("password must be between 8 and 32 charactors long")),
         validators.InputRequired(utils.gettext("password is required"))])
    password_confirm = PasswordField(
        utils.gettext("Repeat password"),
        [validators.InputRequired(utils.gettext("please confirm your password")),
         validators.EqualTo("password", message=utils.gettext("password must match"))])
